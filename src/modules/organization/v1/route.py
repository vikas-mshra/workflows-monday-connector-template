    ...
    try:
        ...
        for content_object_name in content_object_names:
            if content_object_name == "object_types":
                # Filter to change_* mutations and convert to value/label pairs.
                mutations = result["data"]["__type"]["fields"]
                object_types = [
                    {
                        "value": mutation["name"],
                        "label": humanize(mutation["name"]),
                    }
                    for mutation in mutations
                    if mutation["name"].startswith("move_")
                    or mutation["name"].startswith("connect_project_")
                    or mutation["name"].startswith("enroll_items_")
                    or mutation["name"].startswith("change_item")
                ]
