...
def build_selection(return_type, api_token):
    """
    ...
    """
    # GraphQL wraps non-null types in a NON_NULL envelope — peel it off
    # to get to the actual type underneath.
    rt = return_type
    ...
    # Scalars (e.g. JSON, String) and enums have no sub-fields to select.
    if kind in ("SCALAR", "ENUM"):
        return ""

    # For list return types, unwrap to the element type.
    # e.g. [Board!]! -> NON_NULL -> LIST -> NON_NULL -> Board
    if kind == "LIST":
    ...
    if not type_name:
        # Fallback — shouldn't happen with a well-formed schema
        return "{ id name }"

    # Ask Monday.com's introspection API what fields this type has.
    ...
    parts = []
    for f in fields:
        # Peel off NON_NULL wrapper on the field's type
        f_type = f["type"]
        ...
        if f_kind in ("SCALAR", "ENUM"):
            # Plain value — select it directly, e.g. "id", "name"
            parts.append(f["name"])
        elif f_kind == "OBJECT":
