...
from src.monday_client import get_mutation_args, run_monday_file_upload, run_monday_query
    ...
    try:
        ...
        for content_object_name in content_object_names:
            if content_object_name == "object_types":
                # Filter to add_* mutations and convert to value/label pairs.
                mutations = result["data"]["__type"]["fields"]
                object_types = [
                    {
                        "value": mutation["name"],
                        "label": humanize(mutation["name"].removeprefix("add_")),
                    }
                    for mutation in mutations
                    if mutation["name"].startswith("add_")
                ]
